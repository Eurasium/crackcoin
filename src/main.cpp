...
const string strMessageMagic = "crackcoin Signed Message:\n";
...
static const int64 nTargetSpacing = 10; // 10 second blocks, for super fast crack addict speed blocks. 
...
{
    // crackcoin: min work for orphan block takes min work for now
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // crackcoin: continuous target adjustment on every block
...
void ThreadScriptCheck() {
    RenameThread("crackcoin-scriptch");
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // crackcoin: proof of work is checked in ProcessBlock()
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    if (hash != hashGenesisBlock) {
        ...
        // crackcoin: block version starts from 2
        if (nVersion < 2)
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        nTargetInitialLength = 5; // crackcoin: initial prime chain target
        nTargetMinLength = 2;     // crackcoin: minimum prime chain target
    }

    // crackcoin: Generate prime table when starting up
    GeneratePrimeTable();
...
//
// crackcoin: ScanHash is not needed for crackcoin
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (!CheckProofOfWork(pblock->GetHeaderHash(), pblock->nBits, pblock->bnPrimeChainMultiplier, pblock->nPrimeChainType, pblock->nPrimeChainLength))
        return error("crackcoinMiner : failed proof-of-work check");

    //// debug print
    printf("crackcoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("crackcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("crackcoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("crackcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("crackcoin-miner");
	...
	if (!pindexPrev) {
        ...
        if (fDebug && GetBoolArg("-printmining"))
            printf("Running crackcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        // crackcoin: try to find hash that is probable prime
        do
        {
        ...
        if (pblock->nNonce >= 0xffff0000)
            continue;
        // crackcoin: primorial fixed multiplier
        CBigNum bnPrimorial;
        ...
        loop
        {
            ...
            // crackcoin: mine for prime chain
            ...
            if (fNewBlock)
            {
                // crackcoin: a sieve+primality round completes
                // crackcoin: estimate time to block
                ...
                // crackcoin: reset sieve+primality round timer
                nRoundTests = 0;
                nRoundPrimesHit = 0;
    ...
    {
        printf("crackcoinMiner terminated\n");
        throw;
    }
}
