...
static const int64 MAX_MONEY = 2000000000000u * COIN;
...
public:
    ...
    */
    ...
    static bool AllowFree(double dPriority)
    {
        ...
        // need a fee.
        return dPriority > COIN * 720 * 6 / 250; // 720 * 6 blocks found a day.
    }
...
public:
    ...
    unsigned int nTime;
    unsigned int nBits;  // crackcoin: prime chain target, see prime.cpp
    unsigned int nNonce;

    // crackcoin: proof-of-work certificate
    ...
    // crackcoin: header hash does not include prime certificate
    uint256 GetHeaderHash() const
    {
        return Hash(BEGIN(nVersion), END(nNonce));
    }

    // crackcoin: block hash includes prime certificate
    uint256 GetHash() const
    {
...
public:
    ...
    unsigned int nPrimeChainType;   // crackcoin: chain type (memory-only)
    unsigned int nPrimeChainLength; // crackcoin: chain length (memory-only)
    ...
    bool ReadFromDisk(const CDiskBlockPos &pos)
    {
        ...
        // crackcoin: no proof-of-work check here unlike bitcoin
        // Check the header
        return true;
    }
...
public:
    ...
    uint256 nChainWork;
    unsigned int nWorkTransition; // crackcoin: work transition ratio (memory-only)

    unsigned int nPrimeChainType;   // crackcoin: chain type
    unsigned int nPrimeChainLength; // crackcoin: chain length
    int64 nMoneySupply;             // crackcoin: money supply
    ...
    {
        // crackcoin: disabled proof-of-work check for loading block index
...
public:
    uint256 hashPrev;
    uint256 hashBlock; // crackcoin: persist block hash as well
